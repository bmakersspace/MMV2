...
MODEL_SINGLE = "mmv3-ati6o/5"  # Replace with your model ID
MODEL_DOUBLE = "mmv3-ati6o/5"  # Replace with your model ID
...
def process_image():
    ...
    # Draw vertical crop boundaries (cyan)
    #cv2.line(image, (left, 0), (left, image.shape[0]), (255, 255, 0), 2)
    #cv2.line(image, (right, 0), (right, image.shape[0]), (255, 255, 0), 2)
    ...
    for idx, (top, bottom) in enumerate(systems):
        # Draw horizontal system boundaries (magenta)
        #cv2.line(image, (left, top), (right, top), (255, 0, 255), 2)
        #cv2.line(image, (left, bottom), (right, bottom), (255, 0, 255), 2)
        ...
        bars_x = extract_barlines_from_response(data, system_crop_bgr.shape[1], system_crop_bgr.shape[1])
        ...
        for i, x in enumerate(bars_x):
            ...
            cv2.putText(
                ...
                15,          # thickness for border
                cv2.LINE_AA
            )
    
            # Draw white text on top (slightly thinner)
            cv2.putText(
                image,
                str(measure_num),
                pos,
                ...
                1.2,
                (0, 0, 0),  # black text
                2,                 # main text thickness
                cv2.LINE_AA
            )

            #cv2.line(image, (x + left, top), (x + left, bottom), (0, 165, 255), 2)
